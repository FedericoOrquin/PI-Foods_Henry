    ...
    <div className="createContainer">
        <div className="formContainer">
            <div className="formHeader">
                <h1 className="formTitle">Create your own recipe</h1>
                <Link to='/home'><button className="button">Go back</button></Link>
            </div>
            <form action="" className="form">
                ...
                <div className="inputBox">
                    ...
                    {errors.summary && (
                        <p className="errorText">{errors.summary}</p>
                    )}
                </div>
                ...
                <div className="inputBox">
                    ...
                    {errors.healthScore && (
                        <p className="errorText" >{errors.healthScore}</p>
                    )}
                </div>
                <div className="inputBox">
                ...
                <div className="dietsBox">
                    <label htmlFor="diets" className="dietTitle">Diets </label>
                    <div className="dietContainer">
                        {!errorServer ? (diets?.map(function(diet){
                            return(
                                <div key={diet.id} className="dietItem">
                                    <input type="checkbox" name={diet.name} onClick={e=>handleCheck(e)}></input>
                                    <label>{diet.name}</label>
                                </div>
                            )
                        })): (<p className="errorText">No diets found! Error: {errorServer}</p>)}
                    </div>
                </div>       
                <button className={!ableToSubmit ? "buttonSubmit" : "buttonDisabled"} disabled={ableToSubmit} type='submit' onClick={e=>handleSubmit(e)}>Create recipe</button>
            </form>
        </div>
    </div>
  )
